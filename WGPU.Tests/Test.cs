...
namespace WGPU.Tests
{
    ...
    {
        private const bool SelectAdapterByEnumeration = false;
        ...
        public static unsafe void Main(string[] args)
        {
            ...
            if (!SelectAdapterByEnumeration)
            {
                ...
                Console.WriteLine($"Adapter Name: {properties.name}");
                Console.WriteLine($"Adapter Vendor: {properties.vendorName}");
                Console.WriteLine($"Adapter Driver: {properties.driverDescription}");
                Console.WriteLine($"Adapter Backend: {properties.backendType}");
                Console.WriteLine($"Adapter Architecture: {properties.architecture}");

                Console.WriteLine("Adapter Features: " + string.Join(", ", adapter.EnumerateFeatures()));
            }
            else
            {
                Console.WriteLine("\nEnumerating all adapters that support Vulkan:");
                ReadOnlySpan<Adapter> adapters = instance.EnumerateAdapters(Wgpu.InstanceBackend.Primary);
                foreach (var t in adapters)
                {
                    t.GetProperties(out var p);

                    Console.WriteLine($"\nAdapter Name: {p.name}");
                    Console.WriteLine($"Adapter Vendor: {p.vendorName}");
                    Console.WriteLine($"Adapter Driver: {p.driverDescription}");
                    Console.WriteLine($"Adapter Backend: {p.backendType}");
                    Console.WriteLine($"Adapter Architecture: {p.architecture}");

                    Console.WriteLine("Adapter Features: " + string.Join(", ", t.EnumerateFeatures()));
                }
                ...
                {
                    Console.WriteLine("Didn't find adapter for the specified backend!");
                    Environment.Exit(-1);
                }
            }
            ...
            var vertexBuffer = device.CreateBuffer("VertexBuffer", true, (ulong)(vertices.Length * sizeof(Vertex)),
            ...
            var uniformBuffer = device.CreateBuffer("UniformBuffer", false, (ulong)sizeof(UniformBuffer),
